...
Imports group and glyphs names from an `.enc` file. Returns a dictionary with glyph groups, and a list with the order of the groups.
...
Automatically sets unicode values for all glyphs in the font.
...
Automatically sets unicode value(s) for the specified glyph. The method uses RoboFab’s `glyph.autoUnicodes()` function for  common glyphs, and complements it with additional values from `unicodes_extra`.
...
Converts unicode integers to hexadecimal. See also the reverse function `unicode_hexstr_to_int`.
...
Converts a unicode hexadecimal value into an integer. It does exactly the reverse of `unicode_int_to_hexstr`.
