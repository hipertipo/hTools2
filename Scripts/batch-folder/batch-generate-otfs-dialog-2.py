# [h] batch genenerate folder dialog 2
...
class BatchGenerateFolderDialog(BaseWindowController):
    ...
    def __init__(self):
        ...
        self.w.ufos_folder_value = PathBox((10, 40, -15, 22), callback=self.ufo_folder_callback)
    ...
    def ufo_folder_callback(self, sender):
        path = sender.get()
        if path:
            workspace = NSWorkspace.sharedWorkspace()
            workspace.selectFile_inFileViewerRootedAtPath_(path, "")
    
    def _ufos_get_folder_callback(self, paths):
        if paths:
            path = paths[0]
            self.w.ufos_folder_value.set(path)
